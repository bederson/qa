...
class ClusterHandler(BaseHandler):
    def post(self):
        ...
        question_id = self.request.get("question_id")
        question = Question.getQuestionById(question_id)
        cluster_by = self.request.get("cluster_by", CLUSTER_BY_WORDS)
        data = {}
        ...
        if not person:
            ...
            if num_clusters == 1:
                uncluster(question)
                unclusteredIdeas = [ idea.toDict() for idea in Idea.all().filter("question = ", question).order("-date") ]
                data["status"] = 1
                data["cluster_type"] = ""
                data["clusters"] = [{ "name": "Unclustered", "ideas": unclusteredIdeas }]
                
            else:
                try:
                    if cluster_by == CLUSTER_BY_SIMILARITY:
                        include_unclustered = self.request.get("include_unclustered", "0") == "1"
                        clusters = doClusterBySimilarity(num_clusters, question, include_unclustered)
                    else:
                        clusters = doClusterByWords(num_clusters, question)

                    data["status"] = 1
                    data["cluster_type"] = cluster_by
...
class SimilarIdeaHandler(BaseHandler):
    def post(self):
        ...
        question_id = self.request.get("question_id")
        question = Question.getQuestionById(question_id)
        idea_str = self.request.get("idea")
        idea_id = long(idea_str) if idea_str and idea_str.isdigit() else None
        similar_idea_str = self.request.get("similar_idea")
        similar_idea_id = long(similar_idea_str) if similar_idea_str and similar_idea_str.isdigit() else None
        ...
        if not person:
            ...
            if idea_id is not None and similar_idea_id is not None:
...
def getIdeas(questionIdStr):
    ...
    if ideaObjs.count() > 0:
        ...
        for ideaObj in ideaObjs:
            ideas.append(ideaObj.toDict())
        cluster = { "name": "Unclustered", "ideas": ideas }
        clusters.append(cluster)
        
    return { "cluster_type": clusterType, "clusters": clusters }
...
def doClusterByWords(k, question):
    ...
    try:
        ...
        for cluster in clusters:
            clusterObj = Cluster.createCluster("Cluster #" + str(clusterNum + 1), clusterNum, question, CLUSTER_BY_WORDS)
            entry = []
            ideas = []
...
#             row += 1
        ...
        try:
            ...
            clusterNum = 0
            for cluster in clusters:
                clusterObj = Cluster.createCluster("Cluster #" + str(clusterNum + 1), clusterNum, question, CLUSTER_BY_SIMILARITY)
                ideas = []
                i = 0
                for vector in cluster:
